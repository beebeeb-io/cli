...
## Corpus Check
- 31 files · ~42,364 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 318 nodes · 779 edges · 12 communities detected
...
- `was_recently_uploaded()` --calls--> `handle_event()`  [INFERRED]
  src/loopback.rs → src/commands/watch_remote.rs
...
Cohesion: 0.13
Nodes (28): run(), collect_zip_entries(), looks_like_id_prefix(), pull_folder(), pull_single_file(), resolve_as_path(), run(), run_zip() (+20 more)
...
Cohesion: 0.09
Nodes (22): run(), draw_picker(), list(), parse_hours(), pick_share_interactively(), revoke(), run(), run_picker() (+14 more)

### Community 5 - "Community 5"
Cohesion: 0.14
Nodes (26): b64(), compute_file_hash(), create_folder(), do_download(), do_upload(), download_to(), FileEntry, format_size() (+18 more)
...
Cohesion: 0.16
Nodes (18): collect_all_files(), decrypt_chunks_with_binary_key(), detect_key_derivation(), encrypt_chunks_with_string_key(), encrypt_name_with_string_key(), KeyDerivation, repair_file(), repair_folder() (+10 more)

### Community 9 - "Community 9"
Cohesion: 0.17
Nodes (17): decrypt_name(), download_to_mirror(), extract_data(), handle_event(), load_master_key(), next_backoff(), run(), sanitize_filename() (+9 more)
...
- **Thin community `Community 0`** (40 nodes): `ApiClient`, `.check_conflict()`, `.confirm_password()`, `.create_folder()`, `.create_share()`, `.create_stream_token()`, `.delete_share()`, `.download_file()`, `.find_file_by_id_prefix()`, `.get_billing_subscription()`, `.get_billing_usage()`, `.get_file()`, `.get_file_count()`, `.get_json()`, `.get_me()`, `.get_my_region()`, `.get_region()`, `.get_sessions()`, `.get_subscription()`, `.get_usage()`, `.list_files()`, `.list_ops_since()`, `.list_shares()`, `.logout()`, `.move_file()`, `.opaque_login_finish()`, `.opaque_login_start()`, `.open_sync_stream()`, `.ping_health()`, `.require_auth()`, `.signup()`, `.speedtest_download()`, `.speedtest_upload()`, `.stream_url()`, `.trash_file()`, `.upload_encrypted()`, `.url()`, `format_request_error()`, `parse_response()`, `api.rs`
...
- **Why does `run()` connect `Community 5` to `Community 2`, `Community 3`, `Community 4`, `Community 6`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `decrypt_name()` connect `Community 9` to `Community 1`, `Community 2`, `Community 4`, `Community 5`, `Community 7`?**
